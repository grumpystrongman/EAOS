...
| Sandbox tool execution | `tool-execution-service` - `POST /v1/tool-calls`, `GET /v1/tool-calls`, `GET /v1/tool-calls/{toolCallId}` | Runs tool calls under guard rails | Guard rejects unauthorized action, network profile, or approval state |
